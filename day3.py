# -*- coding: utf-8 -*-
"""
Created on Tue Dec  3 19:06:16 2019
...
Line = namedtuple('Line', 'x1 y1 x2 y2 len prev_len')
...
def parse(line):
    ...
    for command in line.split(','):
        ...
        if direction == 'L':
            new_pos = Point(pos.x - length, pos[1])
        if direction == 'U':
            new_pos = Point(pos.x, pos.y - length)
        if direction == 'D':
            new_pos = Point(pos.x, pos.y + length)
        lines.append(Line(pos.x, pos.y, new_pos.x, new_pos.y, length, prev_len))
        pos = new_pos
        prev_len += length
    return lines
...
def check_crossing(line1, line2):
    ...
    if min(horz.x1, horz.x2) < vert.x1 and \
       max(horz.x1, horz.x2) > vert.x1 and \
       min(vert.y1, vert.y2) < horz.y1 and \
       max(vert.y1, vert.y2) > horz.y1:
        return Point(vert.x1, horz.y1)
    else:
        return None


def find_crossings(line1, line2):
    crossings = list()
    idx_l1 = 0
    for segment1 in line1:
        idx_l2 = 0
        for segment2 in line2:
            crossing = check_crossing(segment1, segment2)
            if crossing is not None:
                crossings.append((crossing, idx_l1, idx_l2))
            idx_l2 += 1
        idx_l1 += 1
    return crossings
...
def get_length_till_point(line, idx, point):
    if is_vertical(line[idx]):
        return line[idx].prev_len + abs(line[idx].y1 - point.y)
    else:
        return line[idx].prev_len + abs(line[idx].x1 - point.x)
